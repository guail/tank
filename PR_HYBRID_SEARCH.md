...
> 本功能已提交在 `TANK的英雄笔记`（`flowix` 的个人改版）的 `main` 分支上，
> 由 `flowix search <q> --semantic` / `--hybrid` 启用。如未来要回馈上游 Flowix，可基于此分支开 PR。

## 动机 (Motivation)

当前 `flowix-core` 的检索是纯 **bigram 词面** 倒排索引 (`search.rs`)：
...
- **`EmbeddingProvider` trait** (`flowix-core/src/embed.rs`)：core 保持零 HTTP 依赖，只定义抽象 `embed(&[String]) -> Vec<Vec<f32>>` + `model_name()`。具体实现放在 `flowix-cli`。
...
- `flowix-core/src/embed.rs`（新增）：`EmbeddingProvider`、`SearchMode`、`cosine_similarity`、`rrf_fuse`、`SemanticIndex`、`search_notebooks_hybrid` + 单测（`FakeProvider` / `ConstProvider`）。
- `flowix-core/src/lib.rs`：导出 `pub mod embed;`。
- `flowix-core/src/search.rs`：`MatchField` 加 `Semantic` 变体；`make_snippet` 处理 `Semantic`；`MemoIndex::get_entry` 供语义召回造 snippet。
- `flowix-core/src/service.rs`：新增 `MemoService::search_memos_hybrid`（校验同 `search_memos`，embedding 失败映射为 `FlowixError::Internal`）。
- `flowix-cli/src/embed.rs`（新增）：`OllamaEmbeddingProvider`，走本地 Ollama `/api/embed`（复用工作区已有的 `reqwest` blocking 客户端）。
- `flowix-cli/Cargo.toml` + 工作区 `Cargo.toml`：`reqwest` 开启 `blocking` feature。
- `flowix-cli/src/cli.rs`：`Cli::Search` 加 `mode: SearchMode`；新增 `--semantic` / `--hybrid` flag + 解析 + help。
- `flowix-cli/src/store.rs` + `dispatch.rs` + `mcp.rs`：透传 `mode`；`mode != Lexical` 时构建 `OllamaEmbeddingProvider`（端点 / 模型走 `FLOWIX_OLLAMA_URL` / `FLOWIX_EMBED_MODEL` 环境变量，默认 `http://localhost:11434` / `nomic-embed-text`）后走 `search_memos_hybrid`；MCP 工具描述同步提示语义检索。

## 用法 (Usage)

```bash
# 默认仍是纯词面检索 (向后兼容)
flowix search "rustc command not found"

# 向量 + 词面融合 (推荐)
flowix search "编译器命令找不到" --hybrid

# 纯向量语义检索
flowix search "编译器命令找不到" --semantic

# 自定义 Ollama 端点 / 模型
FLOWIX_OLLAMA_URL=http://127.0.0.1:11434 FLOWIX_EMBED_MODEL=nomic-embed-text \
  flowix search "部署时下载总超时" --hybrid
```

MCP（`flowix_memo` 工具）：命令字符串直接带 `--hybrid` / `--semantic` 即可，例如
