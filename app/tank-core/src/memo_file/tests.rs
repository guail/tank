...
//!   `register_existing_file` / `reconcile_with_disk` /
...
/// `read_notebook_configs` 走 `index.db` (SQLite), `get_memo_base` 通过
/// `set_current_notebook` 走当前 notebook id 而不是 fallback。helper 只负责
/// 给 `MemoFile` 传两个路径 + 切到 "nb_test", 不再写任何
/// legacy JSON。`AtomicUsize` + nanos 后缀防并行 cargo test 撞名。
...
#[test]
fn register_existing_file_does_not_rename_disk() {
    let (mf, base) = fresh_memo_file();
    let abs = base.join("random-name.md");
    fs::write(&abs, "# 我的笔记\nbody").unwrap();
    let memo = mf.register_existing_file(&abs).expect("register ok");
    // v3: 物理文件**不**重命名, 原文件名原样入 memo index
...
//
// 用法上跟其他 ops 测试一样用 `fresh_memo_file` 起 tempdir + 种子 notebook 行。
...
fn notebook_configs_cache_populated_on_first_read() {
    let (mf, _base) = fresh_memo_file();
    // fresh_memo_file 用 `write_notebook_configs` 写过一次 SQLite 行, 但
    // cache 此时还是 None (cache 不会在 new() 自动加载, 等首次 read 触发)。
