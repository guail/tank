...
// 在 14×14 渲染尺寸下, fill 渲染比 stroke 视觉量级更稳 (与 __send / __trash
// 等实心图标同族)。
...
class CodeBlockShikiView implements NodeView {
  ...
  constructor(props: NodeViewRendererProps) {
    ...
    // Keep editor chrome outside the editable <pre>. ProseMirror can only
    // map DOM positions outside contentDOM to the start/end of the node, so
    // mixing controls with <code> makes native selections expand to the end
    // of the code block.
  ...
  private createView() {
    ...
    // Keep the editable subtree first in DOM order. Chrome/ProseMirror's
    // composition handling is sensitive to an empty textblock whose DOM
    // selection sits next to contenteditable=false chrome; putting toolbar
    // siblings before contentDOM can make the first IME composition look like
    // normal text insertion, leaking preedit text into the document.
    this.codePre.classList.add('code-block-editor')
  ...
  private async renderMermaidPreview() {
    ...
    try {
      const { renderMermaidDiagram } = await import('@features/editor/extensions/codeblock-shiki/mermaid-renderer')
...
private unmountFullscreenOverlay(): void {
  ...
  ignoreMutation(mutation: ViewMutationRecord) {
    ...
    if (mutation.target === this.codePre && mutation.type === 'attributes') return true
