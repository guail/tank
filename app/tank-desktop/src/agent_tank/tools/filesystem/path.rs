...
// ============== 4 楂橀宸ュ叿: tokio::fs ==============
//
// 之前 v3 �?"async fn 但函数体从不 .await" 的伪 async, 整个 read/write/edit/ls
// 期间�?Tokio worker 线程卡�?, 单�?工具调用动辄几百 ms ── 期间 UI 滚动 /
// 菜单 / Tiptap 都不响应�?v4 �?tokio::fs 对位, �?worker 真�?能切出去�?// 错�?处理�?`io::Error` 套成 `ToolResult::error(...)`, �?��签名仍是
