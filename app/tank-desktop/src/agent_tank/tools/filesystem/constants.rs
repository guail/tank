...
// ============== limits ==============
//
// �?LLM 调用的�?义上�?(schema `maximum`) 对齐 ── �?LLM 看到�?最�?100
// 条结�?就是真上�? 不�? LLM �?��为能拿到更�?�?实际内部多收 1 条用�?// truncated 标�?�?
...
// glob / grep �?`spawn_blocking`, 内部加�?外硬上限 ── �?LLM �?��
// `limit=1000` 在百万文件目录里�?worker 卡�?�?超出上限就标 truncated
// �?LLM �?�� (缩窄 path / 调高 specificity)�?
...
pub(super) const MAX_GREP_FILE_BYTES: u64 = 4 * 1024 * 1024; // 单文�?> 4MB 跳过
