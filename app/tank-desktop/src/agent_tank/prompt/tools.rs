...
- `available_dirs`: list directories the AI is allowed to access. Returns notebook storage paths (kind=notebook) and user-suggested reference paths (kind=folder), each with absolute `path`. Directories toggled off in File Permissions or missing from disk are excluded. Use this before reading or writing files.
