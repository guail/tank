...
fn cli(args: &[&str]) -> Output {
    Command::new(env!("CARGO_BIN_EXE_tank-cli"))
        .args(args)
        .output()
        .unwrap()
}
...
fn binary_prints_version_and_help() {
    ...
    assert!(stdout(&version).starts_with("tank-cli "));
...
fn binary_reports_usage_errors_with_expected_exit_code() {
    ...
    assert!(stderr(&missing).contains("usage: tank-cli list <notebook>"));
